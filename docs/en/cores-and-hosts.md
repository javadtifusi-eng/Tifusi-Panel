...
1. **Find** — *Node neighbours* reads the names off the certificates of every address in the node's own /24, so it finds sites in the same datacenter (fast, and the approach Xray's authors recommend). After a neighbour scan, **Find more sites** scans the next ring of /24s on either side and lists only names not found before, round after round. *Well-known sites* uses a built-in list of about 160 large sites. *New server (before it's a node)* takes the IP of a server you haven't added yet and gives you a one-line command to run there; it runs the node agent's scanner on that server (with Docker, installed if missing) and reports back to the panel, so a server can be judged before it becomes a node.
2. **Check** — each name must answer with TLS 1.3 and HTTP/2 and a certificate valid for that name. Neighbours are reached by their address, which becomes the `dest` (`IP:443`). Each usable name is then timed from the node, by IP and with no DNS in the timing, one at a time after the checks are over: **ping** is the median of five TCP connects and **TLS** the median of three full handshakes. REALITY waits on that handshake for every new connection, so the list is ordered by it: open from Iran first, then working with chrome, then the shortest handshake.
