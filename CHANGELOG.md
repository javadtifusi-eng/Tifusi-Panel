...
### Tunnel engine
- **Hamrang camouflage transport (TCP or QUIC).** For DPI days: the link mimics a browser session to an allowed domestic site — a Chrome-fingerprinted TLS handshake (uTLS) over TCP, or QUIC with the `h3` ALPN so it reads as HTTP/3. The domestic host is picked in the tunnel form. Spoof stays the tool for a full national-internet cut.
- **Spoof source auto-discovery.** The spoof test can sweep a built-in set of domestic networks in one run and turn the receiver's output into a ready-to-paste source pool for the tunnel to rotate across.
- **Spoof carriers with automatic failover, and advanced stealth.** The spoof tunnel now rides UDP, ICMP Echo or forged TCP, chosen in the tunnel form. **Auto** (the default) tries each and settles on one that works while the Iran side listens on all of them, so a throttled carrier switches over on its own. **Advanced stealth** randomises each packet's TTL, DSCP and source port and accepts only the configured forged source, leaving no fixed shape to fingerprint. Spoof also runs as one multiplexed KCP session now, fixing reconnects that failed with "address already in use" and links that closed each other.
...
### SSL
- **The Let's Encrypt certificate renews itself.** The panel runs `certbot renew` twice a day; a renewed certificate replaces the dashboard's, every IKEv2 Core using it, and Hysteria2's on the nodes. The installer now issues its certificate where the panel can renew it. A failed renewal is reported on Telegram.
