...
### Subscriptions
- **The address customers are given is now separate from the panel's own.** `public_url` did two jobs at once — where the admin reaches the dashboard, and the base every customer's subscription link was built on — so handing customers a second domain moved the dashboard onto it too, and the panel vanished from where its owner expected it. Settings now holds a **Subscription address** beside the panel address; empty falls back to the panel address, exactly as before. The point is that a link handed to customers gets forwarded and eventually filtered, and burning it should not take the dashboard with it.
- **Links already saved in customers' apps keep working and move themselves.** Nothing in any subscription format can rewrite the URL an app has stored — there is no header for it — so a fetch arriving on the panel address is answered with a permanent redirect to the subscription address. Clients follow it and several remember the new one, and `app.json` now carries `subscription_url` so the Tifusi app can store it and follow on its own. The redirect goes out *before* the device limit and on-hold activation run, so one fetch never spends two device slots, and the query string is preserved so `hwid` survives the hop.
- IKEv2 deliberately does not move: its Remote ID and certificate are pinned in every user's imported profile, so changing that address would force all of them to re-import. The panel domain has to keep resolving, and stay on the certificate, for as long as IKEv2 users exist.
...
- Documented in [docs/en/hysteria2.md](docs/en/hysteria2.md), including what tuning cannot fix: Hysteria2 has no forward error correction, so that 30% loss is paid for in retransmissions, and the protocol that attacks it directly is Xray's mKCP — untested on this path, and unlike Hysteria2 it would live inside the panel's existing Core and Inbound machinery. Also recorded as limits: nothing synchronises a Host with the server's own config file, and `install-node.sh` does not install Hysteria2.
