...
### REALITY scanner
- **SNIs from your own users' traffic.** Datacenter neighbours are names MCI has never seen, and it held their upload under 1 Mbps. Each scan now also tries the servers the node's users actually open (read from Xray's access log, named by their certificate; only a count per destination is kept), and the phone test ranks results by upload once one is measured.
