...
- **Настоящие лимиты подключений.** Лимит устройств соблюдается на самих узлах для Xray, IKEv2 и L2TP: подключённые устройства остаются подключены, а лишнее ждёт, пока освободится место.
- **Свой пароль IKEv2/L2TP.** Необязательный пароль входа для каждого пользователя, отдельный от секрета подписки.
- **Любые порты.** Порт HTTPS дашборда настраивается, а в каждом вопросе о порте можно выбрать случайный свободный.
- **Управление узлами.** `tifusi node` на серверах узлов и «Удалить узел» в `tifusi panel`.
- **Исправления.** Узлы, зависавшие в «pending» в редакции с MySQL, падение свежей локальной сборки и данные, оставшиеся после удаления.
...
- **Реселлеры.** Владелец создаёт учётные записи реселлеров на странице Resellers: отдельный вход, разрешённые протоколы и необязательный лимит на число пользователей, общий объём или оба сразу. Реселлер видит и управляет только своими пользователями, выбирает для каждого один из разрешённых протоколов, видит остаток лимита на странице пользователей и не имеет доступа к узлам, хостам, ядрам, туннелям, группам и настройкам.
...
| Панель, профессиональная редакция (MySQL) | 2 vCPU, 2 ГБ RAM, 20 ГБ диска | 2 vCPU, 4 ГБ RAM, 40 ГБ диска |
...
> **Минимальный сервер для панели:** 1 ядро CPU, 1 ГБ RAM, 10 ГБ диска.
> **Для удобной установки и управления:** 2 ядра CPU, 2 ГБ RAM, 20 ГБ диска, Ubuntu 22.04/24.04 или Debian 11/12 на x86_64.
> Панель и узел на одном сервере: не менее 2 ГБ RAM и 25 ГБ диска. Подробнее в разделе «Требования».
> **Профессиональная редакция (MySQL):** 2 ядра CPU, не менее 2 ГБ RAM (рекомендуется 4 ГБ), 20 ГБ диска.

### Панель — стандартная редакция (SQLite)
...
### Панель — профессиональная редакция (MySQL)

```bash
bash -c "$(curl -fsSL https://raw.githubusercontent.com/javadtifusi-eng/Tifusi-Panel/main/install.sh)" -- --pro
```

Та же панель, но все данные (пользователи, узлы, хосты, группы, ядра и настройки) хранятся во встроенном контейнере MySQL 8.4 вместо файла SQLite. Установщик генерирует пароли MySQL, MySQL доступен только во внутренней сети Docker, данные лежат в `mysql-data/`. Подходит для большого числа пользователей или когда нужен стандартный сервер БД для администрирования и резервного копирования; `tifusi panel backup` и `tifusi panel restore` включают дамп MySQL. Узлы устанавливаются одинаково для обеих редакций.
...
Скрипт также ставит `tifusi node` — собственное меню сервера узла с действиями `status`, `logs`, `restart` и `uninstall`, доступными и как `tifusi node <action>`. Удаление узла состоит из двух шагов, поскольку панель никогда не останавливает ничего на сервере узла удалённо: `tifusi node uninstall` на самом узле убирает агента, а пункт **Remove a node from this panel** в `tifusi panel` (или страница **Nodes**) удаляет запись из панели.
...
Все перечисленные порты выбираются при установке и меняются потом командой `tifusi panel port` — на любой вопрос можно ответить `r`, чтобы получить случайный свободный порт. Единственное исключение — `80`: он закреплён, потому что панель отвечает на нём на HTTP-01-проверку Let's Encrypt. Порт HTTPS задаётся переменной `TIFUSI_DASHBOARD_HTTPS_PORT` и по умолчанию равен `443`; для сервера за прокси Cloudflare поставьте `2053`, `2083`, `2087`, `2096` или `8443` и приведите `TIFUSI_PUBLIC_URL` в соответствие, так как адрес `https://host` без порта подразумевает только `443`.
