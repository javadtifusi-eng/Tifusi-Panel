...
class Core(Base):
    ...
    # --- wireguard only ---
    # Served by a dedicated Xray process on the node (node_agent/wireguard.py),
    # not the kernel module: Xray ends each client's TCP on the node and carries
    # it onward with the node's own BBR stack, which on MCI's lossy uplink gave
    # far better uploads than an end-to-end tunnel.
    # UDP 4500 is the default and the point: on MCI the IKEv2/NAT-T port lost
    # ~2% of uplink packets where a random high port lost 10-18%, measured from
    # the same phone to the same server. On a node that also runs IKEv2 the
    # agent shares the port by signature (see node_agent/wireguard.py).
    wireguard_port: Mapped[int | None] = mapped_column(Integer, nullable=True)
    wireguard_mtu: Mapped[int | None] = mapped_column(Integer, nullable=True)
    # The server's key. Generated by the panel, never typed; the public half is
    # derived from it whenever a link is built.
