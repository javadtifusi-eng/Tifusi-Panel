...
Notes for anyone (human or AI coding assistant) working on this codebase.
...
**Editions** — `install.sh` installs the standard edition (SQLite in `data/`); `install.sh --pro` installs the professional edition, which adds `docker-compose.pro.yml` (a MySQL 8.4 service) through `COMPOSE_FILE` in `.env` and points `TIFUSI_DATABASE_URL` at `mysql+aiomysql://`. Every schema change must work on both databases: use `op.batch_alter_table(...)`, drop a foreign key before dropping the column it references, and give every `String` a length. An empty MySQL database is bootstrapped from the models and stamped at head (`app/migrate.py`), because the earliest migrations only ever ran on SQLite. Enum columns use `native_enum=False` so MySQL stores them as VARCHAR like SQLite does. `tifusi panel backup`/`restore` include a `mysqldump` for the professional edition; the Settings page backup is SQLite-only.

**Resellers** — an `Admin` with `is_reseller=True` and `permissions=["users"]` (`app/routers/resellers.py`, owner-only). `app/resellers.py` holds the limits: user count and a volume quota that is the live sum of its users' `data_limit` (no stored balance), checked before a change is added to the session. A user's `protocols` list (None = all) narrows `app/groups/access.py` for links and node configs alike; a reseller's users always carry one. Resellers can't assign groups and are refused by `get_staff_admin` routes (stats, user templates).
